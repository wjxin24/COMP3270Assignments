...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.allPositions:
            self.beliefs[ghostPosition] *= self.getObservationProb(observation, gameState.getPacmanPosition(), ghostPosition, self.getJailPosition())
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        occurance = self.numParticles // len(self.legalPositions)
        remainder = self.numParticles % len(self.legalPositions)
        self.particles= [i for i in self.legalPositions] * occurance
        if remainder > 0:
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        particlesWeight = DiscreteDistribution()
        for particle in self.particles:
            if particle not in particlesWeight:
                particlesWeight[particle] = self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition())
            else:
                particlesWeight[particle] += self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition())
        if particlesWeight.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [particlesWeight.sample() for _ in range(self.numParticles)]
        # raiseNotDefined()
